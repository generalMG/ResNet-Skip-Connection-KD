...
def train(model, loader, optimizer):
    ...
    for batch_idx, (data, target) in enumerate(loader):
        ...
        optimizer.zero_grad()
        output1, output, output_array = model(data)
...
def validate_teacher(model, loader):
    model.eval()
    ...
    with torch.no_grad():
        for batch_idx, (data, target) in enumerate(loader):
            ...
            output_1, output, array = model(data)
            loss = criterion(output, target)
...
for epoch in range(args.epoch):
    print('\nTeacher Epoch: %d' % (epoch+1))
    train_correct, training_loss = train(models_teacher, trainLoader, optimizer_teacher)
    val_correct, validating_loss = validate_teacher(models_teacher, testLoader)
