...
class ResNet(nn.Module):
    ...
    def __init__(self, block, num_layer, num_classes=10, in_dim=3, base_dim=64):
        ...
        if block is Basic:
            ...
            modules_for_export += [Exportable_base(base_dim*2, base_dim*4, down=True)]
            modules1 += [block(base_dim*4, base_dim*4) for _ in range(num_layer[2]-1)]

            modules1 += [block(base_dim*4, base_dim*8, down=True)]
    ...
    def forward(self, x, output_array_t, state, impact):
        batch_size = x.size(0)
        out0 = self.layer0(x)
        out, out_array = self.layer_export(out0)
        out_combined = self.combine(out, out_array, output_array_t, state, impact)
        # comment: had to change combine function output so out_viewed will pass combined x loss
        out1 = self.layer1(out_combined[0])
        out2 = self.avg_pool(out1)
        out3 = out2.view(batch_size, -1)
        out = self.fc_layer(out3)

        # outputting mid data
        out_viewed = out_combined[1].view(batch_size, -1)

        return out_viewed, out, out_array
