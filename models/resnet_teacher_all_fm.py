...
class ResNet(nn.Module):
    ...
    def forward(self, x):
        batch_size = x.size(0)
        out0 = self.layer0(x)
        out_export1 = self.layer_export1(out0)
        out1 = self.layer1(out_export1[0])
        out_export2 = self.layer_export2(out1)
        out2 = self.layer2(out_export2[0])
        out_export3 = self.layer_export3(out2)
        out3 = self.layer3(out_export3[0])

        out4 = self.avg_pool(out3)
        out5 = out4.view(batch_size, -1)
        out = self.fc_layer(out5)

        # outputting mid data
        out_viewed3 = out_export3[1].view(batch_size, -1)
        out_array1 = out_export1[1]

        out_array2 = out_export2[1]
        out_array3 = out_export3[1]

        return out_viewed3, out, out_array1, out_array2, out_array3
