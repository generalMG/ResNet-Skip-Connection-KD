...
class ResNet(nn.Module):
    ...
    def __init__(self, block, num_layer, num_classes=10, in_dim=3, base_dim=64):
        ...
        if block is Basic:
            ...
            modules_for_export1 += [Exportable_base(base_dim, base_dim * 2, down=True)]
            modules1 += [block(base_dim * 2, base_dim * 2) for _ in range(num_layer[1] - 1)]

            modules_for_export2 += [Exportable_base(base_dim * 2, base_dim * 4, down=True)]
            modules2 += [block(base_dim * 4, base_dim * 4) for _ in range(num_layer[2] - 1)]

            modules_for_export3 += [Exportable_base(base_dim * 4, base_dim * 8, down=True)]
            modules3 += [block(base_dim * 8, base_dim * 8) for _ in range(num_layer[3] - 1)]
        ...
        self.layer_export1 = nn.Sequential(*modules_for_export1)
        self.layer1 = nn.Sequential(*modules1)
        self.layer_export2 = nn.Sequential(*modules_for_export2)
        self.layer2 = nn.Sequential(*modules2)
        self.layer_export3 = nn.Sequential(*modules_for_export3)
        self.layer3 = nn.Sequential(*modules3)
    ...
    def combine(self, out, x, t, state, impact):
        if state:
            #print(x)
            x = x + (impact * t)
            #print(x)
    ...
    def forward(self, x, output_array1, output_array2, output_array3, state, impact):
        batch_size = x.size(0)
        out0 = self.layer0(x)

        out1_s, out_array1_s = self.layer_export1(out0)
        out_combined1 = self.combine(out1_s, out_array1_s, output_array1, state, impact)

        out1 = self.layer1(out_combined1)

        out2_s, out_array2_s = self.layer_export2(out1)
        out_combined2 = self.combine(out2_s, out_array2_s, output_array2, state, impact)

        out2 = self.layer2(out_combined2)

        out3_s, out_array3_s = self.layer_export3(out2)
        out_combined3 = self.combine(out3_s, out_array3_s, output_array3, state, impact)

        out3 = self.layer3(out_combined3)

        out_avg = self.avg_pool(out3)
        out_ = out_avg.view(batch_size, -1)
        out = self.fc_layer(out_)

        # outputting mid data
        out_viewed3 = out_array3_s.view(batch_size, -1)

        return out_viewed3, out, out_array1_s, output_array2, out_array3_s
